    ...
    <div class="thanks-wrapper px-3">
        <div class="card">
            <div class="card-body py-5">
                ...
                @if($tenant->logo_path)
                    <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($tenant->logo_path) }}" alt="{{ $tenant->name }}" style="max-height: 40px; max-width: 160px; margin-top: 12px;">
                @else
                    <p class="text-secondary">{{ $tenant->name }}</p>
                @endif
            </div>
        </div>
        <p class="text-center text-secondary mt-3" style="font-size: 0.75rem;">{{ __('Powered by {{ config('app.name') }}') }}</p>
    </div>
</body>
</html>
