@php
    $key = $portal['key'];
    $status = $unit->{$key . '_status'};
    $isLive = $status === 'live';
@endphp
<div class="d-flex align-items-center gap-1">
    @include('inventory._portal-badge', ['status' => $status])
    <form method="POST" action="{{ route('inventory.portal-toggle', $unit) }}" class="d-inline">
        @csrf
        <input type="hidden" name="portal" value="{{ $key }}">
        <button type="submit" class="btn btn-sm btn-icon btn-outline-secondary"
                title="{{ $isLive ? __('Unlist from :portal', ['portal' => $portal['label']]) : __('Mark live on :portal', ['portal' => $portal['label']]) }}">
            @if($isLive)
